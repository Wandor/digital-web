...
describe('Savings Product end to end test', () => {
  it('User Can Create Savings Product', () => {
    /* User logs in */
    cy.visit(`${BASE_URL}`);
    ...
    cy.wait(['@getUserData']);
    cy.visit(`${BASE_URL}/app/savings-products`);
    cy.wait(2000);

    // savings product details
    cy.findByRole('button', { name: /new savings product/i }).click();
    cy.get('#name').type(`test ${faker.random.word()}`);
    cy.get('#GLAccount').type('LOAN').findByText('LOAN').click();
    cy.get('#minimumAmount').type('100');
    cy.findByText(/is enabled/i).click();
    cy.findByRole('button', { name: /next/i }).click();

    // Fees and penalties
    cy.get('#selectedFees > div:nth-child(1)');
    // cy.get('#selectedPenalties').select(['test calculating']);
    cy.findByRole('button', { name: /next/i }).click();

    // confirm and save
    cy.findByRole('button', { name: /save/i }).click();

    cy.url().should('contain', '/app/savings-products');
  });
  ...
  it('User Can View Savings Product', () => {
    ...
    cy.url().should('contain', '/app/view-savings-product/');
    cy.findByRole('button', { name: /next/i }).click();
    cy.findByRole('button', { name: /next/i }).click();
  ...
  it('User Can Edit Savings Product', () => {
    ...
    cy.url().should('contain', '/app/edit-savings-product/');
